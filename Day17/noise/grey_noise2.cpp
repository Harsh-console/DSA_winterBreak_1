...
int main(int argv, char* argc[]){
    ...
    // generate grid
    vector<vector<vector2>> grid_r(numY+1, vector<vector2> (numX+1, vector2 {0, 0}));
    generate_grid(grid_r, numX, numY);
    vector<vector<vector2>> grid_g(numY+1, vector<vector2> (numX+1, vector2 {0, 0}));
    generate_grid(grid_g, numX, numY);
    vector<vector<vector2>> grid_b(numY+1, vector<vector2> (numX+1, vector2 {0, 0}));
    generate_grid(grid_b, numX, numY);


    //find color of each pixel using perlin noise - like function
    vector2 point__ {0, 0};
    float pix_brightness_r = 0;
    float pix_brightness_g = 0;
    float pix_brightness_b = 0;
    ...
    for(int i = 0; i < WIDTH - 1; i++){
        for(int j = 0; j < HEIGHT - 1; j++){
            ...
            //cout<<".";
            pix_brightness_r = Noise2D(point__, GRID_SIZE, grid_r); // give random numbers between -1.0 and 1.0f
            pix_brightness_g = Noise2D(point__, GRID_SIZE, grid_g);
            pix_brightness_b = Noise2D(point__, GRID_SIZE, grid_b);

            //cout<<"*";

            // transform range of [-1.0, 1.0] to the range [0, 255]
            pix_brightness_r += 1.0;
            pix_brightness_r /= 2.0;
            pix_brightness_r *= 255;

            pix_brightness_g += 1.0;
            pix_brightness_g /= 2.0;
            pix_brightness_g *= 255;

            pix_brightness_b += 1.0;
            pix_brightness_b /= 2.0;
            pix_brightness_b *= 255;

            SDL_SetRenderDrawColor(renderer, pix_brightness_r, pix_brightness_g, pix_brightness_b, i+j);
            rect.x = j;
            rect.y = i;
