...
int main(){
    ...
    // move everything to p[ n / 2] = median
    //
    ...
    // now we move p[0], p[1] ,... p[i - 1] into one point
    // and p[i], p[i+1], .. p[n- 1] to another point
    // and we'll need (p[i-1] - p[0]) + (p[n-1] - p[i]) steps
    // = (p[n-1] - p[0]) - (p[i] - p[i-1])
    // so we want to maximize p[i] - p[i-1]
    
    int n, tt;
    cin>>n>>tt;
    set<int> p;
    multiset<int> d;
    //set of all differences: p[1] - p[0], p[2] - p[1], ...
    auto Add = [&](int x) {
        auto ret = p.insert(x);
        assert(ret.second);
        auto it = ret.first;
        if((next(it) != p.end()) && (it!= p.begin())) { // if there is another pile after this and another pile before this, then erase the difference between *next(it) and *prev(it)
